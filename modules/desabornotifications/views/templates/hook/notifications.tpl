{*<div class="logo-banner">*}
{*  <div class="logo-item">*}
{*    <img src="{$module_dir}views/img/1-brand_default.jpg" alt="Night Cocktail Bar" />*}
{*  </div>*}
{*  <div class="logo-item">*}
{*    <img src="{$module_dir}views/img/2-brand_default.jpg" alt="Wine Time" />*}
{*  </div>*}
{*  <div class="logo-item">*}
{*    <img src="{$module_dir}views/img/3-brand_default.jpg" alt="Classic Bar" />*}
{*  </div>*}
{*  <div class="logo-item">*}
{*    <img src="{$module_dir}views/img/4-brand_default.jpg" alt="Original Wine" />*}
{*  </div>*}
{*  <div class="logo-item">*}
{*    <img src="{$module_dir}views/img/5-brand_default.jpg" alt="Cabernet Co." />*}
{*  </div>*}
{*  <div class="logo-item">*}
{*    <img src="{$module_dir}views/img/6-brand_default.jpg" alt="The Cocktail" />*}
{*  </div>*}
{*</div>*}

{*<div class="blog-grid">*}
{*  <div class="blog-card">*}
{*    <div class="blog-image">*}
{*      <img src="https://demo80.leotheme.com/prestashop/vt_winesto_demo/img/leoblog/b/1/9/600_439/b-blog_1.jpg" alt="Grapes">*}
{*      <div class="blog-date">*}
{*        <span>16</span><br>OCT*}
{*      </div>*}
{*    </div>*}
{*    <div class="blog-content">*}
{*      <p class="blog-meta">Sub Category 1 &bull; by: Demo Demo</p>*}
{*      <h3 class="blog-title">Nullam ullamcorper nisl quis ornare molestie</h3>*}
{*    </div>*}
{*  </div>*}

{*  <div class="blog-card">*}
{*    <div class="blog-image">*}
{*      <img src="https://demo80.leotheme.com/prestashop/vt_winesto_demo/img/leoblog/b/1/8/600_439/b-blog_2.jpg" alt="Wine Bottle">*}
{*      <div class="blog-date">*}
{*        <span>16</span><br>OCT*}
{*      </div>*}
{*    </div>*}
{*    <div class="blog-content">*}
{*      <p class="blog-meta">Sub Category 1 &bull; by: Demo Demo</p>*}
{*      <h3 class="blog-title">Turpis at eleifend leo mi elit aenean porta ac sed...</h3>*}
{*    </div>*}
{*  </div>*}

{*  <div class="blog-card">*}
{*    <div class="blog-image">*}
{*      <img src="https://demo80.leotheme.com/prestashop/vt_winesto_demo/img/leoblog/b/1/7/600_439/b-blog_3.jpg" alt="Wine Glasses">*}
{*      <div class="blog-date">*}
{*        <span>16</span><br>OCT*}
{*      </div>*}
{*    </div>*}
{*    <div class="blog-content">*}
{*      <p class="blog-meta">Sub Category 1 &bull; by: Demo Demo</p>*}
{*      <h3 class="blog-title">Morbi condimentum molestie nam enim odio...</h3>*}
{*    </div>*}
{*  </div>*}
{*</div>*}

<div class="info-banner">
  <div class="info-item">

    <span class="info-icon">
      <?xml version="1.0" encoding="utf-8"?>
<svg width="50px" viewBox="0 0 24 24" fill="#3b4456" xmlns="http://www.w3.org/2000/svg">
<path d="M3 9H21M7 15H9M6.2 19H17.8C18.9201 19 19.4802 19 19.908 18.782C20.2843 18.5903 20.5903 18.2843 20.782 17.908C21 17.4802 21 16.9201 21 15.8V8.2C21 7.0799 21 6.51984 20.782 6.09202C20.5903 5.71569 20.2843 5.40973 19.908 5.21799C19.4802 5 18.9201 5 17.8 5H6.2C5.0799 5 4.51984 5 4.09202 5.21799C3.71569 5.40973 3.40973 5.71569 3.21799 6.09202C3 6.51984 3 7.07989 3 8.2V15.8C3 16.9201 3 17.4802 3.21799 17.908C3.40973 18.2843 3.71569 18.5903 4.09202 18.782C4.51984 19 5.07989 19 6.2 19Z" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
    </span>

    <div class="info-text">
      <h3 class="info-title">Wybierz i zbierz</h3>
      <p class="info-subtitle">Nagroda za lojalność</p>
    </div>
  </div>
  <div class="info-item">
    <span class="info-icon">
      <svg fill="#fff" width="50" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
           viewBox="0 0 506.487 506.487" xml:space="preserve">
          <g>
            <g>
              <g>
                <path d="M113.392,355.644c-9.421,0-17.067,7.646-17.067,17.067s7.646,17.067,17.067,17.067c9.421,0,17.067-7.646,17.067-17.067
                  S122.813,355.644,113.392,355.644z"/>
                <path d="M404.087,355.644c-9.421,0-17.067,7.646-17.067,17.067s7.646,17.067,17.067,17.067c9.421,0,17.067-7.646,17.067-17.067
                  S413.508,355.644,404.087,355.644z"/>
                <path d="M498.389,278.844h-91.998c-15.386,0-27.904-12.518-27.904-27.904v-40.363c0-4.71,3.823-8.533,8.533-8.533
                  c4.71,0,8.533,3.823,8.533,8.533v40.363c0,5.973,4.864,10.837,10.837,10.837h55.56l-20.753-83.012
                  c-2.85-11.409-13.065-19.388-24.841-19.388H371.2c11.128-34.389,15.343-64.179,15.744-67.123c0.333-2.441-0.41-4.898-2.031-6.758
                  c-1.621-1.852-3.959-2.918-6.426-2.918H70.724c-4.139,0-7.672,2.97-8.397,7.04l-4.83,27.093h-30.72
                  c-4.71,0-8.533,3.823-8.533,8.533s3.823,8.533,8.533,8.533H54.46l-3.038,17.067h-30.72c-4.71,0-8.533,3.823-8.533,8.533
                  c0,4.71,3.823,8.533,8.533,8.533h27.674l-3.038,17.067h-30.72c-4.71,0-8.533,3.823-8.533,8.533s3.823,8.533,8.533,8.533H42.3
                  l-3.038,17.067H8.533c-4.71,0-8.533,3.823-8.533,8.533c0,4.71,3.823,8.533,8.533,8.533h27.682l-4.275,24.021v0.009L11.136,371.14
                  c-0.469,2.492,0.205,5.069,1.826,7.023c1.613,1.954,4.028,3.081,6.562,3.081h43.435c4.087,24.175,25.114,42.667,50.432,42.667
                  c25.318,0,46.344-18.492,50.432-42.667h189.833c4.087,24.175,25.114,42.667,50.432,42.667c25.318,0,46.345-18.492,50.432-42.667
                  h34.901c9.412,0,17.067-7.654,17.067-17.067v-71.023C506.487,287.172,503.322,281.924,498.389,278.844z M113.391,406.844
                  c-18.816,0-34.133-15.309-34.133-34.133s15.317-34.133,34.133-34.133c18.825,0,34.133,15.309,34.133,34.133
                  S132.215,406.844,113.391,406.844z M351.13,165.018c-12.629,35.038-34.176,75.895-67.763,85.897
                  c-0.375,0.111-0.751,0.222-1.135,0.324c-1.86,0.512-3.763,0.896-5.692,1.212c-0.572,0.094-1.126,0.213-1.698,0.282
                  c-2.372,0.316-4.779,0.512-7.262,0.512H50.509l3.046-17.067h95.249c4.71,0,8.533-3.823,8.533-8.533
                  c0-4.71-3.823-8.533-8.533-8.533H56.593l3.038-17.067h95.258c4.71,0,8.533-3.823,8.533-8.533s-3.823-8.533-8.533-8.533h-92.22
                  l3.046-17.067h95.249c4.719,0,8.533-3.823,8.533-8.533c0-4.71-3.814-8.533-8.533-8.533H68.753l3.038-17.067h95.258
                  c4.71,0,8.533-3.823,8.533-8.533s-3.823-8.533-8.533-8.533h-92.22l3.038-17.067h290.569c-2.423,13.363-7.774,38.81-17.203,65.195
                  C351.206,164.907,351.155,164.958,351.13,165.018z M361.421,312.977c-4.71,0-8.533-3.823-8.533-8.533s3.823-8.533,8.533-8.533
                  h17.067c4.71,0,8.533,3.823,8.533,8.533s-3.823,8.533-8.533,8.533H361.421z M404.087,406.844
                  c-18.825,0-34.133-15.309-34.133-34.133s15.309-34.133,34.133-34.133c18.825,0,34.133,15.309,34.133,34.133
                  S422.912,406.844,404.087,406.844z"/>
              </g>
            </g>
          </g>
        </svg>
    </span>
    <div class="info-text">
      <h3 class="info-title">Dostawa</h3>
      <p class="info-subtitle">Kurier – 25,90 PLN</p>
    </div>
  </div>
  <div class="info-item">
    <span class="info-icon">
      <svg width="50" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <g>
            <path fill="none" d="M0 0h24v24H0z"/>
            <path fill="#fff" d="M22 17.002a6.002 6.002 0 0 1-4.713 5.86l-.638-1.914A4.003 4.003 0 0 0 19.465 19H17a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2h2.938a8.001 8.001 0 0 0-15.876 0H7a2 2 0 0 1 2 2v4a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-5C2 6.477 6.477 2 12 2s10 4.477 10 10V17.002zM20 17v-4h-3v4h3zM4 13v4h3v-4H4z"/>
        </g>
      </svg>
    </span>
    <div class="info-text">
      <h3 class="info-title">Odbiór osobisty</h3>
      <p class="info-subtitle">Z Chmielna 106, Warszawa</p>
    </div>
  </div>
  <div class="info-item">
    <span class="info-icon">
      <svg fill="#fff" width="50" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
           viewBox="0 0 494.055 494.055" xml:space="preserve">
        <g>
          <path d="M483.555,277.333h-4.982c-2.332-36.385-21.757-68.194-50.318-87.453c0.481-1.204,0.753-2.515,0.753-3.891V53.181
            c0-15.077-12.266-27.343-27.343-27.343H27.343C12.266,25.838,0,38.104,0,53.181v234.108c0,15.077,12.266,27.343,27.343,27.343
            H234.97v130.684c0,12.627,10.273,22.9,22.9,22.9h213.284c12.627,0,22.9-10.273,22.9-22.9V287.833
            C494.055,282.034,489.353,277.333,483.555,277.333z M457.526,277.333h-14.622c-3.737-39.971-37.461-71.363-78.392-71.363
            s-74.655,31.393-78.392,71.363h-14.622c3.777-48.009,44.051-85.92,93.014-85.92S453.749,229.323,457.526,277.333z M421.772,277.333
            H307.252c3.64-28.365,27.921-50.363,57.26-50.363S418.133,248.967,421.772,277.333z M21,96.204h387.008v38.653H21V96.204z
             M27.343,46.838h374.321c3.498,0,6.343,2.845,6.343,6.343v22.023H21V53.181C21,49.683,23.846,46.838,27.343,46.838z
             M27.343,293.633c-3.498,0-6.343-2.846-6.343-6.343V155.857h387.008v23.162c-13.415-5.54-28.103-8.606-43.496-8.606
            c-60.546,0-110.24,47.321-114.061,106.92h-4.982c-5.799,0-10.5,4.701-10.5,10.5v5.8H27.343z M473.055,445.316
            c0,1.048-0.853,1.9-1.9,1.9H257.87c-1.048,0-1.9-0.853-1.9-1.9V298.333h217.085V445.316z"/>
          <path d="M339.598,391.051v21.464c0,13.738,11.177,24.914,24.915,24.914c13.738,0,24.914-11.176,24.914-24.914v-21.464
            c12.515-8.287,20.297-22.441,20.297-37.72c0-24.93-20.282-45.212-45.211-45.212c-24.93,0-45.212,20.282-45.212,45.212
            C319.301,368.61,327.082,382.764,339.598,391.051z M364.513,329.12c13.35,0,24.211,10.861,24.211,24.212
            c0,9.447-5.56,18.093-14.163,22.027c-3.737,1.708-6.134,5.44-6.134,9.549v27.607c0,2.158-1.756,3.914-3.914,3.914
            c-2.159,0-3.915-1.756-3.915-3.914v-27.608c0-4.109-2.397-7.84-6.134-9.549c-8.604-3.934-14.163-12.58-14.163-22.026
            C340.301,339.981,351.162,329.12,364.513,329.12z"/>
        </g>
      </svg>
    </span>
    <div class="info-text">
      <h3 class="info-title">Bezpieczna płatność</h3>
      <p class="info-subtitle">Dostosowana do Twoich potrzeb</p>
    </div>
  </div>
</div>


<div class="content-wrapper">
  <div class="left-block">
    <h2>Odwiedź nas na Chmielnej 106</h2>
    <p>Zapraszamy do naszego sklepu stacjonarnego w sercu miasta, gdzie oferujemy szeroki wybór wyselekcjonowanych win, serów i innych przysmaków</p>
    <div class="image-container">
      <img src="{$module_dir}views/img/reight-map.jpg" alt="The Cocktail" />
    </div>
  </div>
  <div class="right-block">
    <div class="above-map-image">
      <img src="{$module_dir}views/img/gg.JPG" alt="The Cocktail" />
    </div>
    <div class="map-container" style="height: 250px; width: 100%;">
      <iframe
              src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2443.007877550159!2d20.9984942!3d52.2288965!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x471ecd067273c509%3A0x92a092b1d9dea55d!2sDesabor%20Warsaw%20%7C%20Spanish%20Gourmet%20%26%20Wine!5e0!3m2!1sen!2spl!4v1698784787952!5m2!1sen!2spl"
              width="100%"
              height="100%"
              style="border:0;"
              allowfullscreen=""
              loading="lazy"
              referrerpolicy="no-referrer-when-downgrade"
      ></iframe>
    </div>



{*    <div class="map-container" style="height: 500px; width: 100%;">*}
{*      <!-- Контейнер карты -->*}
{*      <div id="map" style="height: 100%; width: 100%;"></div>*}
{*    </div>*}

{*    <!-- Подключаем Google Maps API -->*}
{*    <script*}
{*            src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAOJfIm50TpUI4BwiplwQ-JLx27-Iyhk88&callback=initMap"*}
{*            async*}
{*            defer*}
{*    ></script>*}

{*    <script>*}
{*      // Функция инициализации карты*}
{*      function initMap() {*}
{*        // Координаты для метки (пример для Варшавы)*}
{*        const location = { lat: 52.2297, lng: 21.0122 };*}

{*        // Создаем карту*}
{*        const map = new google.maps.Map(document.getElementById("map"), {*}
{*          center: location,*}
{*          zoom: 14, // Уровень приближения*}
{*        });*}

{*        // Добавляем метку на карту*}
{*        const marker = new google.maps.Marker({*}
{*          position: location,*}
{*          map: map,*}
{*          title: "Desabor",*}
{*        });*}
{*      }*}
{*    </script>*}




    {*    <div class="map-container">*}
{*      <gmp-map*}
{*              id="map"*}
{*              center="52.2297,21.0122"*}
{*              zoom="10"*}
{*              map-id="DEMO_MAP_ID"*}
{*              style="height: 100%"*}
{*      ></gmp-map>*}
{*    </div>*}

{*    <script*}
{*            src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAOJfIm50TpUI4BwiplwQ-JLx27-Iyhk88&loading=async&libraries=maps&v=beta"*}
{*            defer*}
{*    ></script>*}

{*    <script>*}
{*      function initMap() {*}
{*        const center = { lat: 52.2297, lng: 21.0122 }; // Центр карты — Варшава*}

{*        // Создаем карту*}
{*        const map = new google.maps.Map(document.getElementById("map"), {*}
{*          center: center,*}
{*          zoom: 10,*}
{*          mapId: "DEMO_MAP_ID",*}
{*        });*}

{*        // Добавляем маркер в Варшаве*}
{*        const marker = new google.maps.Marker({*}
{*          position: center,*}
{*          map: map,*}
{*          title: "Варшава", // Текст, который появится при наведении на маркер*}
{*        });*}
{*      }*}

{*      // Ждем загрузки карты, чтобы инициализировать маркер*}
{*      window.initMap = initMap;*}
{*    </script>*}

  </div>
</div>


